...
def build_cayley_graph_custom_generators(n):
    G = nx.Graph()
    ...
    generators = [Permutation([[0, 1, 2, 3]]), Permutation([[0, 1]])]
    gen_labels = ['(1 2 3 4)', '(1 2)']
...
def visualize_3d_graph(G, gen_labels):
    ...
    color_map = {'(1 2 3 4)': 'red', '(1 2)': 'blue'}
    edge_traces = []
    ...
    node_trace = go.Scatter3d(
        ...
        name=r"$\text{elements in } S_4$"
    )
    fig = go.Figure(data=edge_traces + [node_trace])
    fig.update_layout(
        #title=r"$\text{Cayley Graph of }S_4 \text{with generator set of one transposition and one 4-cycle}$",
        showlegend=True,
    ...
    plot(fig, filename='cayley_graph_S4_gs_2.html', auto_open=False, include_mathjax='cdn')

n = 4
G, gen_labels = build_cayley_graph_custom_generators(n)
